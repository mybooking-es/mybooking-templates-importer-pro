...
<div class="ocdi  wrap  about-wrap">
	...
	<?php ob_start(); ?>
		<h1 class="ocdi__title" style="margin-bottom: 1rem"><?php esc_html_e( 'Mybooking Templates Pro', 'pt-ocdi' ); ?></h1>
	<?php
	...
	<div class="ocdi__intro-text">
		<p class="about-description">
			<?php esc_html_e( 'Importing Mybooking pro templates is the easiest way to get a renting site ready.', 'pt-ocdi' ); ?><br>
			<?php esc_html_e( 'Once you installed a template all pages, images, styles and theme settings will be imported to your site.', 'pt-ocdi' ); ?>
		</p>
		<h3><?php esc_html_e( 'Before you begin', 'pt-ocdi' ); ?></h3>
		<p>
			<?php esc_html_e( 'You need to install the following components:', 'pt-ocdi' ); ?>
			<ol>
				<li><?php esc_html_e( 'Mybooking Theme', 'pt-ocdi' ); ?></li>
				<li><?php esc_html_e( 'Mybooking Reservation Engine plugin', 'pt-ocdi' ); ?></li>
				<li><?php esc_html_e( 'Elementor plugin', 'pt-ocdi' ); ?></li>
			</ol>
		<hr>
	</div>
	...
	<ul>
		<li><?php esc_html_e( 'Existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'pt-ocdi' ); ?></li>
	...
	<?php if ( ! empty( $this->import_files ) ) : ?>
		<?php if ( empty( $_GET['import-mode'] ) || 'manual' !== $_GET['import-mode'] ) : ?>
			<a class="ocdi__gl-item-button  button" href="<?php echo esc_url( add_query_arg( array( 'page' => $this->plugin_page_setup['menu_slug'], 'import-mode' => 'manual' ), menu_page_url( $this->plugin_page_setup['parent_slug'], false ) ) ); ?>" class="ocdi__import-mode-switch"><?php esc_html_e( 'Switch to manual import!', 'pt-ocdi' ); ?></a>
		<?php else : ?>
			<a class="ocdi__gl-item-button  button" href="<?php echo esc_url( add_query_arg( array( 'page' => $this->plugin_page_setup['menu_slug'] ), menu_page_url( $this->plugin_page_setup['parent_slug'], false ) ) ); ?>" class="ocdi__import-mode-switch"><?php esc_html_e( 'Switch back to theme predefined imports!', 'pt-ocdi' ); ?></a>
		<?php endif; ?>
	<?php endif; ?>
	<br>
	<hr>
